...
void dibujar_matriz(int ini_x, int ini_y)
{
    for (int fila = 2; fila < FILAS; fila++) // Las FILAS ARRANCAN EN 2 PORQUE LAS 2 PRIMERAS SON INVISIBLES
    {
        for (int col = 0; col < COLUMNAS; col++)
        {
            ...
            int y = ini_y + (fila - 2) * TAM_CELDA; // RESTAMOS A FILAS 2 YA QUE 2 SON INVISIBLES
...
void dibujar_rectangulo(int x, int y, int ancho, int alto, int color)
{
    for (int i = 0; i < alto; i++)
    {
        for (int j = 0; j < ancho; j++)
        {
            ...
            gbt_dibujar_pixel(x + j, y + i, color); //Esta dibujando el cuadrado que va a caer la ficha
        }
    }
}
